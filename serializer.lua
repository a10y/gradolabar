-- AST code serialization, from lua-fish https://github.com/davidm/lua-fish
...
local function indentBody(body)
  local out = ""
...
function funcs.Do(ast)
  return 'do ' .. indentBody(convert(ast[1])) .. ' end'
end

function funcs.While(ast)
  return 'while ' .. convert(ast[1]) .. ' do \n' .. indentBody(convert(ast[2])) .. ' end'
end

function funcs.Repeat(ast)
  return 'repeat ' .. indentBody(convert(ast[1])) .. ' until ' .. convert(ast[2])
end
...
function funcs.If(ast)
  ...
  ts[#ts+1] = ' then\n'
  ts[#ts+1] = indentBody(convert(ast[2]))
  local i=3; while i <= #ast do
    if i < #ast then
      ts[#ts+1] = ' elseif '
      ts[#ts+1] = convert(ast[i])
      ts[#ts+1] = ' then\n'
      ts[#ts+1] = indentBody(convert(ast[i+1]))
      i = i + 2
    else
      ts[#ts+1] = ' else\n'
      ts[#ts+1] = indentBody(convert(ast[i]))
      i = i + 1
    end
  end
  ts[#ts+1] = ' end'
